...
serve(async (req)=>{
  ...
  try {
    ...
    const totalAdded = users?.filter((u)=>u.subscription_status === "added").length || 0;
    const totalActive = users?.filter((u)=>u.subscription_status === "active").length || 0;
    ...
    if (users?.length) {
      ...
      lastMonthRevenue = payments.filter((p)=>new Date(p.paid_at) >= oneMonthAgo).reduce((sum, p)=>sum + Number(p.amount), 0) || 0;
    }
    ...
    const lastMonthConversions = users?.filter((u)=>u.converted_at && new Date(u.converted_at) >= oneMonthAgo).length || 0;
    ...
    return new Response(JSON.stringify({
      ...
      revenue: {
        total: Number(partner.total_revenue.toFixed(2)),
