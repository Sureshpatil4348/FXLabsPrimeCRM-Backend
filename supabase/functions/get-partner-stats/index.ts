...
serve(async (req) => {
    ...
    try {
        ...
        const { data: partner, error: partnerError } = await supabase
            .from("crm_partner")
            .select(
                "email, full_name, commission_percent, is_active, created_at, total_revenue, total_converted"
            )
            .eq("id", partnerId)
            .single();
        ...
        const { data: users, error: usersError } = await supabase
            ...
            .select(
                "subscription_status, converted_at, created_at, region, user_id"
            )
            .eq("crm_partner_id", partnerId);
        if (usersError) {
            console.error(
                "Users fetch error:",
        ...
        const totalAdded =
            users?.filter((u) => u.subscription_status === "added").length || 0;
        const totalActive =
        ...
        if (users?.length) {
            ...
            lastMonthRevenue =
                payments
                    .filter((p) => new Date(p.paid_at) >= oneMonthAgo)
        ...
        const recentSignups =
            users?.filter((u) => new Date(u.created_at) >= thirtyDaysAgo)
                .length || 0;
        const lastMonthConversions =
            users?.filter(
                (u) => u.converted_at && new Date(u.converted_at) >= oneMonthAgo
            ).length || 0;
        // Calculate conversion rate
        const conversionRate =
            totalUsers > 0
                ? Number(
                      ((partner.total_converted / totalUsers) * 100).toFixed(2)
                  )
                : 0;
        return new Response(
            JSON.stringify({
