...
serve(async (req) => {
  ...
  try {
    ...
    // Query crm_user_metadata by email for subscription info and block status
    const { data: metadata, error: dbError } = await supabaseClient
      .from("crm_user_metadata")
      .select("user_id, subscription_status, subscription_ends_at, is_blocked")
      .eq("email", email)
      .single();
    ...
    const { user_id, subscription_status, subscription_ends_at, is_blocked } = metadata;

    // Check if user is blocked - return expired immediately
    if (is_blocked) {
      console.log(`User with email ${email} is blocked - returning expired status`);
      return new Response(
        JSON.stringify({
          subscription_status: "expired"
        }),
    ...
    if (subscriptionEndDate < now) {
      ...
      const { data: updateData, error: updateError } = await supabaseClient
        ...
        .eq("user_id", user_id)
        .neq("subscription_status", "expired") // Only update if not already expired
      ...
      if (updateError) {
        ...
        console.log(`Updated subscription status to expired for user ${user_id} (${email})`);
      }
