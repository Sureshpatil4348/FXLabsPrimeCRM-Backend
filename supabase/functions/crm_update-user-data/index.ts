...
serve(async (req)=>{
  ...
  try {
    ...
    // Fetch existing user metadata - ✅ Added converted_at to SELECT
    const { data: existingUser, error: fetchError } = await supabase
      .from("crm_user_metadata")
      .select("user_id, email, region, subscription_ends_at, subscription_status, is_blocked, converted_at")
    ...
    // ✅ Handle subscription_ends_at update WITH intelligent status update
    if (validated.subscription_ends_at) {
      ...
      if (!oldDate || newDate.getTime() !== oldDate.getTime()) {
        ...
        // ✅ Smart status update based on converted_at
        if (existingUser.converted_at) {
          // User has paid before → set to "paid"
          updateData.subscription_status = "paid";
          console.log(`Subscription extended and status set to 'paid' (user has payment history)`);
        } else {
          // User has never paid → set to "trial"
          updateData.subscription_status = "trial";
          console.log(`Subscription extended and status set to 'trial' (no payment history)`);
        }
    ...
    const { data: updatedUser, error: updateError } = await supabase
      ...
      .update(updateData)
      .eq("user_id", validated.user_id)
      .select("user_id, email, region, subscription_ends_at, subscription_status, is_blocked, updated_at")
      .single();
    ...
    return new Response(JSON.stringify({
      ...
      changes: {
        ...
        subscription_status_updated: !!updateData.subscription_status,
